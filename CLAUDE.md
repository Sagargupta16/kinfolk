...
Built and verified (114 tests, typecheck, lint, build): schema, fusion + layout pipeline, generation bands, relation overlay, connection density (presence rings), server-side contact visibility filtering, canvas with staggered entrance / hover focus / fusion reveal, three levels of detail, search, the census-driven legend, design tokens, demo mode, `/signin`, `/tree` with both data sources.
