...
- **Stroke weight is ONE scale, in `--kf-stroke-*` tokens, and four readers depend on it** (the skeleton rule, the base relation rule, the three closeness rules, and the legend's sample lines). Before this they each named their own number and had already drifted: the base relation rule said `1` while `is-close-1` said `0.75`, so an edge's weight depended on whether its closeness class had matched, and 0.75 lost its dash rhythm entirely below about 0.8 zoom -- the channel saying "acquaintance" was destroying the channel saying which KIND. Now faint 1.1 / soft 1.5 / firm 1.9 / skeleton 2.3, evenly spaced so the scale is learnable, with the skeleton always heaviest so a relation can never outweigh a parentage.
- **Direction is a tapered stroke (`#kf-taper`), never an arrowhead.** `MarkerType.ArrowClosed` was removed for three independent reasons: a 14px triangle on a 400px bezier lands where the line meets a card and reads as part of the card's border; React Flow hoists markers into a shared `<defs>` and writes the colour as an INLINE style, so the head could not inherit dimming, the active state or the past colour (a dimmed edge kept a full-contrast head); and it was redundant with the dash flow, which is legible along the whole line. The gradient must use `gradientUnits="objectBoundingBox"` so ONE def serves paths running in every direction -- `userSpaceOnUse` would need per-edge coordinates. `is-active` and `is-ended` override the `url()` reference rather than the colour, because a gradient stroke cannot be recoloured. `var()` DOES resolve inside `<stop stop-color>` (verified on a live element: both stops compute to `rgb(134,141,155)`).
...
Not built yet: the graph editor, invite/share flow, merge-proposal UI, contact and relation editor UI. Sign-in still cannot complete end to end: it needs a GitHub OAuth app, which is dashboard-only (github.com/settings/developers) -- `AUTH_GITHUB_ID` and `AUTH_GITHUB_SECRET` are the only empty variables left in `.env.local`. Everything up to the GitHub redirect is exercised; the callback, `createUser`, and the provisioned first canvas are verified by direct database calls rather than by driving a real OAuth round trip.

Not visually confirmed in the last session: the Browser pane would not composite (`document.visibilityState === "hidden"`), so edge paths read 0 and no screenshot could be taken. The CSS was verified instead by injecting matching elements into the live `.react-flow` container and reading computed styles, which is what caught the four-way stroke-weight drift.
